...
void init(){
    ...
    for(int i=1;i<=n;i++){
        ...
        p[i].pos+=qwq;
        maxx=max(maxx,p[i].pos);
    }
    st = new long long [maxx<<2];
    ...
    for(int i=1;i<=n;i++){ 
        ...
        switch(p[i].opr){
            ...
            case 3:{
                       ans=rankk(x,1,1,maxx)+1;//找的是x前面数的排名 所以最后+1才是 x的排名
                       printf("%lld\n",ans);
                       break;
                   }
            case 4:{
                        ans=(kth(x-qwq,1,1,maxx)-qwq);
                       printf("%lld\n",ans);
                       break;
                   }
            case 5:{
                       ans=(pre(x,1,1,maxx)-qwq);
                       printf("%lld\n",ans);
                       break;
                   }
            case 6:{
                       ans=(nex(x,1,1,maxx)-qwq);
                       printf("%lld\n",ans);
                       break;
                   }
